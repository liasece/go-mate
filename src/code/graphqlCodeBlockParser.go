...
var (
	...
	GraphqlBlockTypeEnum         CodeBlockType = CodeBlockType{"enum", true, `(?s)^\s*((extend\s+)?\s*enum\s+(\w+)\s*(implements\s+\w+\s*)?\s*(\{\s*(.*?)\s*\}))\s*$`, 1, 3, 6, nil, "\n", "{}", 5, false}
	GraphqlBlockTypeEnumField    CodeBlockType = CodeBlockType{"enum_field", true, `(?s)^\s*((\w+)(\s*@[^\n]*)*)\s*$`, 1, 2, -1, []string{"enum"}, "\n|,", "()", 1, true}
)
